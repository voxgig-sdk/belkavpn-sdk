...
import (
	...
	omni "github.com/voxgig-sdk/belkavpn-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BelkavpnError); is {
				out := map[string]any{
					"name":    "BelkavpnError",
...
func omniCtx(arg any, client *sdk.BelkavpnSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BelkavpnSDK); is {
			client = live
