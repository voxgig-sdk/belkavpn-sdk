...
{
    private static function utility(): BelkavpnUtility
    {
        return new BelkavpnUtility();
    }
    ...
    private static function ctx(array $over = []): BelkavpnContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new BelkavpnContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new BelkavpnOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof BelkavpnError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new BelkavpnOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = BelkavpnMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new BelkavpnOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = BelkavpnMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new BelkavpnOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = BelkavpnMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BelkavpnSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = BelkavpnMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new BelkavpnResponse([]),
            'result' => new BelkavpnResult([]),
        ]);
        [, $err] = BelkavpnMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new BelkavpnSpec([]),
            'response' => null,
            'result' => new BelkavpnResult([]),
        ]);
        [, $err] = BelkavpnMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new BelkavpnSpec(['step' => 's']),
            'response' => new BelkavpnResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new BelkavpnResult([]),
        ]);
        [, $err] = BelkavpnMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new BelkavpnSpec(['step' => 's']),
            'response' => new BelkavpnResponse(self::resp(200, ['v' => 1])),
            'result' => new BelkavpnResult([]),
        ]);
        [, $err] = BelkavpnMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new BelkavpnSpec(['step' => 's']),
            'response' => new BelkavpnResponse(self::resp(200, ['v' => 2])),
            'result' => new BelkavpnResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        BelkavpnMakeResponse::call($ctx);
    ...
    {
        $preset = new BelkavpnResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new BelkavpnSpec([]),
            'response' => new BelkavpnResponse([]),
            'result' => new BelkavpnResult([]),
        ...
        [$got, $err] = BelkavpnMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new BelkavpnResult([])]);
        [, $err] = BelkavpnMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new BelkavpnSpec([]), 'result' => null]);
        [, $err] = BelkavpnMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BelkavpnSpec(['step' => 's']),
            'result' => new BelkavpnResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new BelkavpnOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BelkavpnMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BelkavpnSpec(['step' => 's']),
            'result' => new BelkavpnResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new BelkavpnOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BelkavpnMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BelkavpnResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new BelkavpnSpec([]),
            'result' => new BelkavpnResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = BelkavpnMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BelkavpnMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new BelkavpnError('boom', 'boom');
        $utility->fetcher = function (BelkavpnContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BelkavpnSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = BelkavpnMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (BelkavpnContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = BelkavpnMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(BelkavpnResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (BelkavpnContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BelkavpnSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = BelkavpnMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new BelkavpnResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new BelkavpnSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = BelkavpnMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = BelkavpnMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = BelkavpnMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new BelkavpnResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, BelkavpnDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new BelkavpnResult(['ok' => false])]);
        $this->expectException(BelkavpnError::class);
        BelkavpnDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new BelkavpnResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', BelkavpnMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        BelkavpnMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new BelkavpnBaseFeature();
        $b = new BelkavpnBaseFeature();
        BelkavpnFeatureAdd::call($ctx, $a);
        BelkavpnFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): BelkavpnBaseFeature
    {
        $f = new BelkavpnBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        BelkavpnFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): BelkavpnContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new BelkavpnSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BelkavpnPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new BelkavpnResponse(['headers' => null]),
            'result' => new BelkavpnResult([]),
        ]);
        BelkavpnResultHeaders::call($ctx);
