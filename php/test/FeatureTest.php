...
class FtCtrl extends BelkavpnControl
{
...
class FtClient
{
    ...
    private BelkavpnUtility $utility;

    public function __construct(BelkavpnUtility $utility, array $options)
    {
    ...
    public function get_utility(): BelkavpnUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BelkavpnContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BelkavpnContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BelkavpnContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BelkavpnConfig::make_feature($name);
    ...
    private static function buildUrl(BelkavpnSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BelkavpnError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BelkavpnResponse($fetched);
        }
    ...
    private function fail(BelkavpnContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BelkavpnContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
