...
class BelkavpnMakeError
{
    public static function call(?BelkavpnContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new BelkavpnContext([], null);
        }
        $op = $ctx->op ?? new BelkavpnOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new BelkavpnResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof BelkavpnError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new BelkavpnError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof BelkavpnError) {
