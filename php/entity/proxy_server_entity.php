...
// Belkavpn SDK ProxyServer entity
...
class ProxyServerEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ProxyServerEntity($this->_client, $opts);
    }

    /**
     * @param ProxyServer|array $args ProxyServer data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = BelkavpnHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return ProxyServer|array The current ProxyServer data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of ProxyServer fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = BelkavpnHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of ProxyServer fields).
     */
    ...
    /**
     * List ProxyServer items matching the given filter.
     *
     * @param ProxyServerListMatch|array|null $reqmatch Match filter (any subset
     *   of ProxyServer fields) as an assoc-array; ProxyServerListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ProxyServer[]|array A list of ProxyServer items as assoc-arrays at
     *   the SDK boundary; throws BelkavpnError on failure (item-5 convention).
     */
