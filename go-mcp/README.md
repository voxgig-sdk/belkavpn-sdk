...
[MCP](https://modelcontextprotocol.io) server exposing the Belkavpn SDK as
two agent tools — `belkavpn_list` and `belkavpn_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/belkavpn-mcp)
make build

# 2. Provide credentials via the environment
export BELKAVPN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user belkavpn \
  -- /absolute/path/to/belkavpn-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./belkavpn-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// belkavpn_list: first page of records
{ "entity": "proxy_server" }
{ "entity": "proxy_server", "query": { } }

// belkavpn_load: one record by id
{ "entity": "proxy_server", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/belkavpn-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user belkavpn \
     -- "$PWD"/dist/*/belkavpn-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `belkavpn_list` and `belkavpn_load` tools now appear
   in new sessions. Ask the agent to *"list proxy_server using belkavpn"*
   and it calls `belkavpn_list` with `{"entity":"proxy_server"}`.
...
```sh
export BELKAVPN_APIKEY=sk_live_xxx            # API key
export BELKAVPN_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `belkavpn_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `belkavpn_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BELKAVPN_APIKEY` | API key sent with every request. |
| `BELKAVPN_BASE` | Optional override of the API base URL. |
...
```sh
./belkavpn-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"belkavpn_load","arguments":{"entity":"proxy_server","query":{"id":1}}}}'
```
