...
class ProxyServerEntity
  ...
  def make
    opts = @_entopts.dup
    ProxyServerEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BelkavpnHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [ProxyServer, Hash] the current ProxyServer data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BelkavpnHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of ProxyServer fields)
  def match_get
  ...
  #
  # @param reqmatch [ProxyServerListMatch, Hash, nil] match filter (any subset of
  #   ProxyServer fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<ProxyServer>, Array] the matching ProxyServer items; raises BelkavpnError on failure
