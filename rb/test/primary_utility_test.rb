...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = BelkavpnOmni.make_runner(TEST_JSON_FILE, BelkavpnSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : BelkavpnOmni.errify(err)
    end
