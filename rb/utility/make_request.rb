...
module BelkavpnUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = BelkavpnResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BelkavpnResponse.new(fetched)
    else
