...
require_relative 'Belkavpn_types'
...
class BelkavpnSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = BelkavpnHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = BelkavpnHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, BelkavpnFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = BelkavpnHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = BelkavpnHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = BelkavpnHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = BelkavpnHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = BelkavpnError.new(
        "graphql_error", "BelkavpnSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.ProxyServer.list / client.ProxyServer.load({ "id" => ... })
  def ProxyServer(data = nil)
    require_relative 'entity/proxy_server_entity'
    ProxyServerEntity.new(self, data)
  end
