...
BelkaVPN API client, generated from the OpenAPI spec.

The BelkaVPN API allows users to bypass geographical restrictions and website filters, ensuring access to blocked social media platforms while traveling abroad.

Learn more about BelkaVPN API at [github.com/l0v3m0n3y/belkavpn](https://github.com/l0v3m0n3y/belkavpn).

This is an unofficial SDK for the BelkaVPN public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — ProxyServer — that you
...
const items = await client.ProxyServer().list()
```
...
// proxyservers is an array of ProxyServer entities, populated with mock data
// — call proxyservers[0].data() for the record itself
...
```go
client := sdk.Test()
result, err := client.ProxyServer(nil).List(
    nil, nil,
)
```
...
// List all proxyservers (returns ProxyServerEntity[] — .data() for the record)
const proxyservers = await client.ProxyServer().list()
...
```bash
cd go-mcp && go build -o belkavpn-mcp .
```
...
```json
{
  "mcpServers": {
    "belkavpn": {
      "command": "/abs/path/to/belkavpn-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **ProxyServer** | The ProxyServer entity (list). | `/socks` |
...
# List all proxyservers (returns a list, raises on error)
proxyservers = client.ProxyServer().list()
...
// List all proxyservers (returns an array; throws on error)
$proxyservers = $client->ProxyServer()->list();
print_r(array_map(fn($item) => $item->data_get(), $proxyservers));
```

### Golang

```go
import sdk "github.com/voxgig-sdk/belkavpn-sdk/go"

client := sdk.New()

// List all proxyservers
...
# List all proxyservers (returns an Array; raises on error)
proxyservers = client.ProxyServer.list
puts proxyservers
```
...
- Upstream API: [https://github.com/l0v3m0n3y/belkavpn](https://github.com/l0v3m0n3y/belkavpn)
...
Generated from the BelkaVPN API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
