...
boru-driven command-line client **and** interactive REPL for the Belkavpn
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/belkavpn-cli)
make build
...
./belkavpn-cli list proxy_server

# 5. Override the API base URL for a single call
BELKAVPN_BASE=https://api.example.com ./belkavpn-cli list proxy_server

# 6. No arguments -> interactive REPL
./belkavpn-cli
belkavpn> list proxy_server
belkavpn> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/belkavpn-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/belkavpn-cli list proxy_server
   ```
...
Run with no arguments to open a REPL (prompt `belkavpn>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `proxy_server`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/belkavpn-cli`. |
...
(`./belkavpn-cli <expr>`) and interactively (the REPL), and expressions compose the
