// Command Belkavpn-cli is a CLI + REPL for the Belkavpn SDK,
...
const prompt = "belkavpn"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "proxy_server"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BELKAVPN_APIKEY (api key), BELKAVPN_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BELKAVPN_APIKEY carries the API key and
	// BELKAVPN_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BELKAVPN_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BELKAVPN_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBelkavpnSDK(opts)
