...
from belkavpn_sdk.belkavpn_types import (
    ProxyServer,
    ProxyServerListMatch,
)
...
class ProxyServerEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ProxyServerEntity(self._client, opts)
    ...
    def match_get(self) -> ProxyServer:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[ProxyServer]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.ProxyServer().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
