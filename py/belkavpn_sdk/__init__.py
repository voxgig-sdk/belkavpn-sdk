...
from belkavpn_sdk.utility import register

# Load features
from belkavpn_sdk.feature.base_feature import BelkavpnBaseFeature
from belkavpn_sdk.features import _has_feature, _make_feature
...
class BelkavpnSDK:
    ...
    def get_utility(self):
        return BelkavpnUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("BelkavpnSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def ProxyServer(self, data=None) -> "ProxyServerEntity":
        """Entity factory: client.ProxyServer().list() / client.ProxyServer().load({"id": ...})."""
        from belkavpn_sdk.entity.proxy_server_entity import ProxyServerEntity
        return ProxyServerEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "BelkavpnSDK":
        if sdkopts is None:
            sdkopts = {}
