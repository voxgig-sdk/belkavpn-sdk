...
class BelkavpnTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BelkavpnTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BelkavpnTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BelkavpnTestRunner.getenv("BELKAVPN_TEST_LIVE")
        override = BelkavpnTestRunner.getenv("BELKAVPN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BelkavpnTestRunner.getenv(key)
        ...
        explain = BelkavpnTestRunner.getenv("BELKAVPN_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["BELKAVPN_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if BelkavpnTestRunner._test_control is not None:
            return BelkavpnTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BelkavpnTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BelkavpnTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BelkavpnTestRunner.load_test_control()
...
def load_env_local():
    BelkavpnTestRunner.load_env_local()


def env_override(m):
    return BelkavpnTestRunner.env_override(m)


def entity_data(v):
    return BelkavpnTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BelkavpnTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BelkavpnTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BelkavpnTestRunner.load_test_control()


def live_client_options():
    return BelkavpnTestRunner.live_client_options()


def live_delay_ms():
    return BelkavpnTestRunner.live_delay_ms()
