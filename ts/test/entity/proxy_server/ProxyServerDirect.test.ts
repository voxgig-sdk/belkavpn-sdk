...
describe('ProxyServerDirect', async () => {
  ...
  test('direct-list-proxy_server', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-proxy_server', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['BELKAVPN_TEST_PROXY_SERVER_ENTID']
