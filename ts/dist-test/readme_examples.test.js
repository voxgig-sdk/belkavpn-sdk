...
const TEST_SEED = { "entity": { "proxy_server": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+BelkavpnSDK\b/.test(code) ||
        /\bBelkavpnSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+BelkavpnSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/BelkavpnSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
